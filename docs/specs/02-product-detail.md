...
Give the customer all the information they need to make a purchase decision for a specific product, including real-time stock status. Age verification and store operating hours rules apply on this page in the same way as the catalog.
...
| I-04 | Adjust quantity | Uses the +/− controls to set how many units to add, up to the product's available stock |
| I-05 | Tap "Add to cart" | Adds the product and quantity to the cart |
| I-06 | Tap a related product | Navigates to that product's detail page |
...
| O-07 | Price | The product's fixed price; no variant-based price changes |
| O-08 | Stock badge | **Available** (green), **Low stock ≤5** (yellow), **Out of stock** (red/grey), **Coming soon** (grey) |
| O-09 | "Add to cart" button | Enabled only when the product's stock > 0 |
| O-10 | Cart counter update | The cart icon badge increments immediately after adding |
| O-11 | Confirmation feedback | A brief toast or banner confirms the item was added |
| O-12 | Related products section | A row of products from the same category or brand, shown at the bottom |
...
| Product coming soon | Admin has marked it as coming soon | Button hidden; grey badge shown; no purchase possible |
...
- The quantity selector cannot exceed the product's available stock.
- If a product goes out of stock between page load and the moment the customer taps "Add to cart," the system blocks the action and informs the customer.
