...
// Wrapped in a function (instead of top-level statements) so this file can
// double as an Ionic CLI hook: @ionic/cli's hook loader does
// `require(hookPath)` and then calls the module's default export as a
// function — a module with only top-level side effects satisfies the
// require() but fails the "must export a function" check and aborts the
// ionic command. See ionic.config.json ("hooks": build:before/serve:before).
